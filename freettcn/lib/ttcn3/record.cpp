...
freettcn::ttcn3::CTypeRecord::CValue::CValue(CTypeRecord &type):
...
freettcn::ttcn3::CTypeRecord::CValue::CValue(const CValue &src):
  CType::CValue(src), _fieldNames(src._fieldNames), _fieldMap(src._fieldMap)
{
}


const ORG_ETSI_TTCN3_TCI::TciValue &freettcn::ttcn3::CTypeRecord::CValue::getField(const Tstring &p_field_name) const
{
...
void freettcn::ttcn3::CTypeRecord::CValue::setField(const Tstring &p_field_name, const TciValue &p_new_value)
{
  auto it = _fieldMap.find(&p_field_name);
  if(it == _fieldMap.end())
...
void freettcn::ttcn3::CTypeRecord::CValue::setFieldOmitted(const Tstring &fieldName)
{
  ...
  it->second.reset(new CValueOmitted(val.getType(), val.getValueEncoding(), val.getValueEncodingVariant()));
}


Tboolean freettcn::ttcn3::CTypeRecord::CValue::operator==(const RecordValue &rec) const
{
...
freettcn::ttcn3::CTypeRecord::CTypeRecord(const TciModuleId  &moduleId,
                                       const Tstring      &name,
